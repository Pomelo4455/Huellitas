...
router.get("/", async (req,res)=> {
    try {
        ...
        if (!allCampaigns.length) res.status(200).send({Error: 'No hay campañas'});
...
router.get("/:id", async (req, res) => {
    ...
    try {
        if (id) {
            ...
            detailCampaign.length ? res.status(200).send(detailCampaign) : res.status(404).send(`Esta campaña no existe o fue eliminada`);
        }
    } catch (error) {
        res.status(404).send({error: error.message});
    }
